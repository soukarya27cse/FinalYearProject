...
const PriceTooltip = ({ active, payload, label, currency }) => {
  ...
  return (
    <div style={{
      background: '#0f1420', border: '1px solid #253650', borderRadius: 6,
      padding: '0.55rem 0.85rem', fontFamily: 'IBM Plex Mono, monospace',
      fontSize: '0.66rem', color: '#d4dce8', boxShadow: '0 4px 20px rgba(0,0,0,0.6)',
      minWidth: 150,
    }}>
      <div style={{ color: '#445566', marginBottom: '0.3rem', fontSize: '0.58rem' }}>{label}</div>
      {payload.filter(p => p.value != null).map((p, i) => (
        <div key={i} style={{ color: p.color, marginBottom: 2, display: 'flex', justifyContent: 'space-between', gap: '1rem' }}>
          <span style={{ color: '#7a91aa' }}>{p.name}</span>
    ...
    <div className="chart-wrap">
      ...
      <ResponsiveContainer width="100%" height={50}>
        <ComposedChart data={filtered} margin={{ top: 0, right: 12, left: 0, bottom: 0 }}>
          ...
          <Tooltip
            contentStyle={{ background: '#0f1420', border: '1px solid #253650', borderRadius: 6, fontFamily: 'IBM Plex Mono, monospace', fontSize: '0.62rem' }}
            formatter={v => v != null ? [`${(v/1e6).toFixed(2)}M`, 'Volume'] : ['—', 'Volume']}
          />
          <Bar dataKey="volume" fill={COLORS.volume} opacity={0.85} name="Volume" />
        </ComposedChart>
      </ResponsiveContainer>
    </div>
  )
